'''
Reads the MIT GraphChallenge files and constructs edgelist files for dynamic graph applications.
Please read the script and understand and modify as needed.
'''
...
def write_graph_to_file(filename, graph_in, nodes_in):
    # Write the graph to the file
    with open(filename, 'w') as file:
        # Write the number of nodes and edges on the first two lines
        file.write(f"{nodes_in} {nodes_in}\n")
        file.write(f"{len(graph_in.edges)}\n")
...
def In_Degree_Distribution(G):
    ...
    s = pd.Series(degree)
    percentiles = [0.10, 0.25, 0.50, 0.75, 0.95]
...
for increment in range(1, 11):
    incremented_filename = f"{filename}_{increment}.tsv"
    print("File: ", incremented_filename)

    with open(incremented_filename, 'r') as file:
        """ for _ in range(2):  # Skip the first two lines
            next(file) """
        for line in file:
            src, dest, weight = line.split()
            G.add_edge(int(src)-1, int(dest)-1, weight=int(weight))
    ...
    write_graph_to_file(output_filename, G, number_nodes)
