...
#SBATCH -J sssp-am-cca
#SBATCH -p general
#SBATCH -o am-sssp_%j.txt
#SBATCH -e am-sssp_%j.err
  ...
  for CHIP_DIM in "${CHIP_DIM_VALUES[@]}"; do
	...
	CUTOFF=${RHIZOME_CUTOFF[$RHIZOME]}
            srun -N 1 -n 1 -c ${threads} ${exe} -r ${repository} -d ${dataset} \
                 ...
                 -chip_dim ${CHIP_DIM} \
                 -memory_cc ${memory} -rhizomes ${RHIZOME} -cutoff ${CUTOFF} -action_queue_size ${ACTION_QUEUE_SIZE} -script run_sssp_am.zsh
    done
  done
done
