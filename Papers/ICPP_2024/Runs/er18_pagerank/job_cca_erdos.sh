...
#SBATCH -J pr-erdos-cca
#SBATCH -p general
#SBATCH -o erdos-pr_%j.txt
#SBATCH -e erdos-pr_%j.err
  ...
  for CHIP_DIM in "${CHIP_DIM_VALUES[@]}"; do
	...
	CUTOFF=${RHIZOME_CUTOFF[$RHIZOME]}
            srun -N 1 -n 1 -c ${threads} ${exe} -r ${repository} -d ${dataset} \
                 ...
                 -chip_dim ${CHIP_DIM} \
                 -memory_cc ${memory} -rhizomes ${RHIZOME} -cutoff ${CUTOFF} -action_queue_size ${ACTION_QUEUE_SIZE} -script run_pagerank_erdos.zsh
    done
  done
done
