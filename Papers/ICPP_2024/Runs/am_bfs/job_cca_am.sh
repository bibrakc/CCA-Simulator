...
#SBATCH -J bfs-am-cca
#SBATCH -p general
#SBATCH -o am-bfs_%j.txt
#SBATCH -e am-bfs_%j.err
...
#change directory to:
cd ${repository}/Papers/ICPP_2024/Runs/am_bfs
...
MEMORY_CC[16]=3148000
VICINITY[16]=1
MAX_EDGES[16]=20

MEMORY_CC[32]=1112000
VICINITY[32]=1
MAX_EDGES[32]=20
  ...
  for CHIP_DIM in "${CHIP_DIM_VALUES[@]}"; do
	...
	CUTOFF=${RHIZOME_CUTOFF[$RHIZOME]}
            srun -N 1 -n 1 -c ${threads} ${exe} -r ${repository} -d ${dataset} \
                 ...
                 -chip_dim ${CHIP_DIM} \
                 -memory_cc ${memory} -rhizomes ${RHIZOME} -cutoff ${CUTOFF} -action_queue_size ${ACTION_QUEUE_SIZE} -script run_bfs_am.zsh
    done
  done
done
