...
AAP_PATH="$REPO_PATH/Applications/Page_Rank_Fixed_Iterations_Rhizome"
...
run_cmake() {
    ...
    -D SPLIT_QUEUES=true \
    -D ACTIONQUEUESIZE=100000 \
    -D DIFFUSE_QUEUE_SIZE=32768 \
    -D WEIGHT="$7"
  ...
  for THROTTLE in "${THROTTLE_VALUES[@]}"; do
    for RECVBUFFSIZE in "${RECVBUFFSIZE_VALUES[@]}"; do
        for TERMINATION in "${TERMINATION_VALUES[@]}"; do
            ...
            # Run the experiment
            "$REPO_PATH/r22_pagerank/$SCRIPT_TO_RUN" \
	    -hx "$CHIP_SIZE_X" \
	    -hy "$CHIP_SIZE_Y" \
