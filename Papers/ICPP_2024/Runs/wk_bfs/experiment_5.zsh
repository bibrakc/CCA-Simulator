...
AAP_PATH="$REPO_PATH/Applications/Breadth_First_Search_Rhizome"
# Create a function to run cmake with the given parameters
run_cmake() {
    CC="$CC_EXE" \
    CXX="$CXX_EXE" \
    cmake -S "$AAP_PATH" -B build_${8}_R${5}_${9} \
    ...
    -D SPLIT_QUEUES=true \
    -D ACTIONQUEUESIZE="$ACTION_QUEUE_SIZE" \
    -D DIFFUSE_QUEUE_SIZE="$DIFFUSE_QUEUE_SIZE" \
    -D WEIGHT="$7"
    cmake --build build_${8}_R${5}_${9} -j 8
}
  ...
  for THROTTLE in "${THROTTLE_VALUES[@]}"; do
    for RECVBUFFSIZE in "${RECVBUFFSIZE_VALUES[@]}"; do
        for TERMINATION in "${TERMINATION_VALUES[@]}"; do
            ...
            run_cmake "$THROTTLE" "$RECVBUFFSIZE" "$TERMINATION" "$ThrottleTime" "$RHIZOMES" "$CUTOFF" "false" "$CHIP_SIZE_X" "$TRAIL"
            # Run the experiment
            "$REPO_PATH/wikipedia/$SCRIPT_TO_RUN" \
	    -hx "$CHIP_SIZE_X" \
	    -hy "$CHIP_SIZE_Y" \
