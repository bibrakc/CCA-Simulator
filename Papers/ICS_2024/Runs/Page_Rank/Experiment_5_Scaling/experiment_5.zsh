...
for THROTTLE in "${THROTTLE_VALUES[@]}"; do
    for RECVBUFFSIZE in "${RECVBUFFSIZE_VALUES[@]}"; do
        for TERMINATION in "${TERMINATION_VALUES[@]}"; do
            ...
            run_cmake "$THROTTLE" "$RECVBUFFSIZE" "$TERMINATION" "$ThrottleTime" 1
            # Run the experiment
            "$REPO_PATH/Papers/ICS_2024/Runs/Page_Rank/$SCRIPT_TO_RUN" -hx "$CHIP_SIZE_X" -hy "$CHIP_SIZE_Y" -dataset "$DATASET_PATH" -network "$NETWORK" -m "$MEMORY_CC" -iter 12
        done
    done
done
